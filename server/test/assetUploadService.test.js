...
function createHarness(options = {}) {
  ...
  const repository = {
    async createPendingUpload(input) {
      ...
      storedUpload = {
        ...
        quarantineObjectKey: input.quarantineObjectKey,
        cleanObjectKey: null,
  ...
  const storageProvider = {
    getSafetyAttestation() {
      return options.storageAttestation || safeStorageAttestation();
    },
    ...
    async statQuarantineObject() {
      return options.stat || { exists: true, byteLength: options.upload?.declaredByteLength || 3 };
    },
  ...
  const service = createAssetUploadService({
    organizationAuthorization: {
      async requireRole(organizationId, actorId, role) {
        roleCalls.push({ organizationId, actorId, role });
      },
    },
...
test('unsafe storage attestation prevents upload record creation', async () => {
  const harness = createHarness({
    storageAttestation: { ...safeStorageAttestation(), publicReadDisabled: false },
  });
