...
async function reserveCapabilitiesForScanWithClient(
  client,
  { organizationId, scanId, requirements },
) {
  if (!client || typeof client.query !== 'function') {
    throw new TypeError('Usage reservation requires an active PostgreSQL client.');
  }
  ...
  for (const requirement of normalized) {
    ...
    const existingReservation = await client.query(
      `select id, units, status, period_start
        ...
        limit 1
        for update`,
      [organizationId, scanId, requirement.capability],
    );
...
async function finalizeReservationsForScanWithClient(
  client,
  { organizationId, scanId, outcome },
) {
  if (!client || typeof client.query !== 'function') {
    throw new TypeError('Usage finalization requires an active PostgreSQL client.');
  }
...
function createEntitlementRepository(pool) {
  ...
  async function reserveCapabilitiesForScan(input) {
    const client = await pool.connect();
    try {
      await client.query('begin');
      const result = await reserveCapabilitiesForScanWithClient(client, input);
  ...
  async function finalizeReservationsForScan(input) {
    const client = await pool.connect();
    try {
      await client.query('begin');
      const result = await finalizeReservationsForScanWithClient(client, input);
      await client.query('commit');
      return result;
    } catch (error) {
