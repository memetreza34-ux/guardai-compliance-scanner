...
- `004_target_verification_challenges_draft.sql` — backend-only DNS TXT ownership challenges, expiration and attempt limits; **not an applied migration**.
...
11. verify worker-only/challenge tables are unavailable to browser roles,
...
17. verify only successful unexpired DNS challenges can set a Target to `verified`,
18. verify challenge attempt/expiry limits and admin-only backend routes,
19. generate TypeScript database types,
20. commit the generated migration and types,
21. only then promote through the normal staging → production release process.
...
- Verification challenge rows are backend-only and intentionally have no browser RLS policy/grant.
...
- queue/challenge state is server/worker-only,
...
- DNS TXT target-verification challenge lifecycle,
...
The browser never receives `DATABASE_URL`, database passwords, challenge token hashes or worker lease identities.
