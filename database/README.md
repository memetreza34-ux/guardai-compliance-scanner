...
4. consolidate/review the approved SQL design into that generated migration,
...
14. generate TypeScript database types,
15. commit the generated migration and types,
16. only then promote through the normal staging → production release process.
...
The GuardAI API/worker path uses repository contracts backed by native PostgreSQL transactions. Current repository work includes:
...
- tenant-scoped Scan + Job status reads.
